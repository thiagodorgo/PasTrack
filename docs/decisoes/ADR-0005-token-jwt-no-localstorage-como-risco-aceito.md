...
- Situação em 15/09/2026: a expiração de 8 h, a CSP e a coluna `versao_token` estão na `main`. A entrega de identidade (#21) recarrega o usuário a cada requisição, confere a versão do token e responde 401 com `codigo: "SESSAO_INVALIDA"` para sessão revogada. Com ela, o `it.fails` do usuário desativado vira teste normal. A base do frontend, do PR #20, mantém o token no `localStorage`, confere a expiração dele no próprio navegador e continua sem nenhum dos sinks citados.
